...
@Service
public class AdaptiveActionScorerImpl
        implements AdaptiveActionScorer {

    @Override
    public List<ActionUtility> rankActions(
            VerificationContext context) {

        List<ActionUtility> actions =
                new ArrayList<>();

        for (VerificationAction action
                : VerificationAction.values()) {
            ...
            VerificationSignalType signalType =
                    mapToSignalType(action);

            if (signalType == null) {
                continue;
            }

            if (context.completedSignals()
                    .contains(signalType)) {
                continue;
            }
    ...
    @Override
    public ActionUtility bestAction(
            VerificationContext context) {
    ...
    private double calculateInformationGain(
        ...
        /*
         * The more uncertain the current case,
         * the more valuable additional evidence becomes.
         */
        return baseGain
                * (0.5 + context.uncertainty());
    }
    ...
    private VerificationSignalType mapToSignalType(
        ...
        return switch (action) {
            ...
            case EXTERNAL_VERIFICATION ->
                    VerificationSignalType.EXTERNAL_VERIFICATION;
    ...
    private String buildReason(
        ...
        return action
                ...
                + "for the current uncertainty level.";
    }
}